...
//	}
	...
	def GlobalVarDuplicate(VariableAssignment varAss){
		var base = EcoreUtil2.getContainerOfType(varAss, Program)
	...
	public static val WRONG_PARAMETER_COUNT = 'wrongParamaterCount'
	
	@Check
	def ValidateExternalParameterCount(ExternalUse use){
		var parent = use.ref
		if (parent.parameters.length() !== use.exp.length()){
			error('Call to external function with incorrect number of parameters', MathPackage.Literals.EXTERNAL_USE__EXP, WRONG_PARAMETER_COUNT)
		}
	}
